...
- `src/lib/period.ts`: 실제 export는 `ResetType`(`daily` | `weekly_mon` | `weekly_thu`), `currentPeriodKey(resetType, now?)`, `RESET_LABEL`, 그리고 KST 시간 공용 헬퍼 `kstParts(now)`(연/월/일/요일, 일=0)·`kstMidnight(y,m,d)`(해당 KST 날짜 00:00의 Date) **다섯 가지**다. `RESET_LABEL` 값은 `daily → "매일 00시 초기화"`, `weekly_mon → "월요일 00시 초기화"`, `weekly_thu → "목요일 00시 초기화"`. 이 모듈은 순수 함수/상수라 서버·클라이언트 어디서든 값으로 import 가능하다. **KST 날짜/자정 계산이 필요하면 Intl 변환을 새로 만들지 말고 반드시 이 헬퍼를 재사용한다**(시간대 로직 복제 금지).
- `src/lib/presets.ts`: `ChecklistCategory`(`daily` | `weekly` | `boss`), `PresetItem`(`id`, `name`, `category`, `reset_type`), `PRESET_ITEMS`, `CATEGORY_LABEL`(`daily → "일일 컨텐츠"`, `weekly → "주간 퀘스트"`, `boss → "주간 보스"`), `CATEGORY_ORDER = ["daily","weekly","boss"]`. 이 모듈도 순수 모듈이라 클라이언트에서 값 import 가능. **주의: `PRESET_ITEMS`엔 daily/weekly만 있다(id `d1..d5`, `w1..w3`). 주간 보스(category `"boss"`)는 코드 상수가 아니라 DB 테이블 `boss_presets`(관리자 CRUD, 전체 유저 공통)에서 서버가 조회해 내려준다** — 보스 목록 화면은 `PRESET_ITEMS` + 서버가 전달한 boss_presets 조회 결과를 합쳐 `CATEGORY_ORDER` 순으로 그린다. `completions`/`quest_durations`/`character_boss_selection`의 `item_id`(text)는 이 `PresetItem.id` 또는 `boss_presets.id`와 매칭된다. 자세한 컬럼·RLS는 `supabase/README.md` 참고.
